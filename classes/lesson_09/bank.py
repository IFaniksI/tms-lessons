# Наша цель сегодня: сделать консольный эмулятор банковского приложения, в котором можно будет создавать свои счета,
# совершать платежи, переводить деньги между счетами.
# Наше приложение будет общаться с пользователем через консоль, каждый раз спрашивая у пользователя,
# каким будет следующее действие.
# Программа будет до тех пор, пока пользователь не введёт 0, это будет означать действие "Выйти из программы".
# В нашей программе мы почти не будет обрабатывать ошибки, которые может допустить пользователь.
# Для простоты считаем что пользователь знает что делает и всё вводит корректно.
